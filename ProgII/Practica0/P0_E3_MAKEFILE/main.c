 ...
 *    RESPUESTA:
 *      Lo que ocurre es que ejecuta el programa hasta donde esta el brakpoint. En el momento en el que llega a donde esta el breakpoint la ejecución 
 *      se detiene. Al continuar la ejecución con Step Into [F11]  y llegar a la linea de la llamada a sqrt() abre el archivo sqrt.c y, al ir dándole 
 *      sucesivamente a F11, muestra la ejecucion de sqrt paso a paso, mostrando los valores de todas las variables, incluidas las de la función absoluto. 
 *      Cuando termina la ejecución de sqrt vuelve al main y continua mostrando la ejecución del main paso a paso y mostrando el valor de cada variable hasta 
 *      llegar a la invocación de gcd donde vuelve a ocurrir lo mismo que con la invocación de sqrt(), la muestra paso a paso.
 * 
 ...
 *      ejecución de la función absoluto y se retoma la ejecución de la función sqrt desaparece la pestaña de absoluto ya que el valor de retotno de esta función se ha guardado
